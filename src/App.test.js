...
test("search is made on by clicking on search button and result found - test 1", async () => {
  ...
  fetchMock.getOnce(
    ...
    JSON.stringify({
      ...
      data: [
        ...
        {
          Title: "Beat Feet: Scotty Smileys Blind Journey to Ironman",
          Year: 2015,
          imdbID: "tt5117146",
        },
      ],
  ...
  await waitFor(() => {
    ...
    expect(results.childNodes[1]).toHaveTextContent(
      "Beat Feet: Scotty Smileys Blind Journey to Ironman"
    );
    expect(queryByTestId("no-result")).toBe(null);
  });
});
...
test("search is made on by clicking on search button and result found - test 2", async () => {
  ...
  fetchMock.getOnce(
    ...
    JSON.stringify({
      ...
      data: [
        ...
        { Title: "Lard and the Peace Maze", Year: 2010, imdbID: "tt5046522" },
        {
        ...
        { Title: "Harry & Snowman", Year: 2010, imdbID: "tt2898306" },
      ],
    })
  );
  fireEvent.input(input, {
  ...
  await waitFor(() => {
    ...
    expect(results.childNodes[2]).toHaveTextContent(
      "Macau Stories III: City Maze"
    );
    expect(results.childNodes[3]).toHaveTextContent(
